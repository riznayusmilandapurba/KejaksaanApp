...
import 'package:kejaksaan/pages/p_alirankepercayaandetail.dart';
import 'package:kejaksaan/pages/p_alirankepercayaanedit.dart';
import 'package:kejaksaan/pages/p_pegawaidetail.dart';
import 'package:kejaksaan/pages/rating.dart';
import 'package:url_launcher/url_launcher.dart';
import 'package:google_fonts/google_fonts.dart';

class AliranKepercayaanList extends StatefulWidget {
  const AliranKepercayaanList({Key? key}) : super(key: key);

  @override
  State<AliranKepercayaanList> createState() => _AliranKepercayaanListState();
}

class _AliranKepercayaanListState extends State<AliranKepercayaanList> {
  ...
  Future<void> _getPengaduanpegawai() async {
    try {
      http.Response res = await http.get(Uri.parse('http://192.168.0.102/kejaksaan_server/pengawasanGET.php'));
      if (res.statusCode == 200) {
        List<Datum> p_pegawaiList = ModelPengawasanFromJson(res.body).data ?? [];
        setState(() {
                        ...
                        children: [
                          IconButton(
                            ...
                            onPressed: (){
                              Navigator.push(
                                  context,
                                  MaterialPageRoute(builder: (context) => AliranKepercayaanDetail(data)),
                                );
                            }
                          ),
                          IconButton(
                            ...
                            onPressed: data.status == 'approve' || data.status == 'reject'
                                ...
                                : () {
                                      Navigator.push(
                                  context,
                                  MaterialPageRoute(builder: (context) => PengawasaAliranKepercayaanEdit(
                                      id: data.id,
