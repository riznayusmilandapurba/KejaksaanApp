...
import 'package:kejaksaan/models/modeltindakkorupsi.dart';
...
class TindakKorupsiList extends StatefulWidget {
  const TindakKorupsiList({Key? key}) : super(key: key);

  @override
  State<TindakKorupsiList> createState() => _TindakKorupsiListState();
}

class _TindakKorupsiListState extends State<TindakKorupsiList> {
  ...
  Future<void> _getPengaduanpegawai() async {
    try {
      http.Response res = await http.get(Uri.parse('http://192.168.0.102/kejaksaan_server/tindakkorupsiGET.php'));
      if (res.statusCode == 200) {
        List<Datum> p_pegawaiList = ModelTindakkorupsiFromJson(res.body).data ?? [];
        setState(() {
    ...
    if (p_pegawai.status != 'approve' && p_pegawai.status != 'reject') {
      ...
      try {
        http.Response res = await http.post(
          Uri.parse('http://192.168.0.102/kejaksaan_server/tindakkorupsiDEL.php'),
                        ...
                        children: [
                          IconButton(
                            ...
                            onPressed: (){
                              Navigator.push(
                                  context,
                                  MaterialPageRoute(builder: (context) => TindakKorupsiDetail(data)),
                                );
                            }
                          ),
                          IconButton(
